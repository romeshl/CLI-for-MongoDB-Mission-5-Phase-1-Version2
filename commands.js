...
const Seed_Data = JSON.parse(
	await readFile(
		new URL("./Seed_Data/trademe.listings.json", import.meta.url)
	) // Read the seed data from the JSON file stored in the Seed_Data folder
);
...
export async function handleShowCommand(options) {
...
export async function handleUpdateCommand(id) {
...
export async function handleDeleteCommand(options) {
	...
	try {
		...
		if (options.all) {
			...
			try {
				...
				Show_Error(
					`Unable to delete listings. Error: ${error.message}`
				);
			}
		}
		db.connection.close();
